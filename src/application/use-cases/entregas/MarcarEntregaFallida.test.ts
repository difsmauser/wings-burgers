...
import { MarcarEntregaFallida } from './MarcarEntregaFallida';
...
describe('MarcarEntregaFallida', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new MarcarEntregaFallida(entregaRepo, pedidoRepo, geoService);
  });

  it('marca una entrega en camino como fallida con motivo', async () => {
    await useCase.ejecutar('entrega-1', 'Cliente no disponible');

    expect(entregaRepo.actualizar).toHaveBeenCalledWith(
      expect.objectContaining({
        id: 'entrega-1',
        estado: EstadoEntrega.FALLIDO,
        motivoNoEntrega: 'Cliente no disponible',
      })
    );
    expect(pedidoRepo.actualizar).toHaveBeenCalledWith('pedido-1', { estado: 'entrega_fallida' });
  ...
  it('lanza RecursoNoEncontradoError si la entrega no existe', async () => {
    ...
    await expect(useCase.ejecutar('no-existe', 'motivo')).rejects.toThrow(RecursoNoEncontradoError);
  });
  ...
  it('lanza TransicionEstadoInvalidaError si la entrega no está en camino', async () => {
    ...
    await expect(useCase.ejecutar('entrega-1', 'motivo')).rejects.toThrow(TransicionEstadoInvalidaError);
  });

  it('registra la fecha de completado al marcar como fallida', async () => {
    await useCase.ejecutar('entrega-1', 'Dirección incorrecta');

    const updateCall = vi.mocked(entregaRepo.actualizar).mock.calls[0][0];
    expect(updateCall.completadaEn).toBeInstanceOf(Date);
  });
});
